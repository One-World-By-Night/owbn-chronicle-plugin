...
if (!function_exists('accessSchema_client_local_check_access')) {
    function accessSchema_client_local_check_access($email, $role_path, $client_id, $include_children = true)
    {
        $payload = [
            'email'            => sanitize_email($email),
            'role_path'        => sanitize_text_field($role_path),
        ...
        if (!is_array($data) || !array_key_exists('granted', $data)) {
            return new WP_Error('invalid_response', 'Invalid response from local access check.');
        }
