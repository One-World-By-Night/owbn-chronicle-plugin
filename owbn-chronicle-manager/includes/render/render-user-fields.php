...
function owbn_render_user_info($key, $value, $meta) {
    ...
    echo "<option value=\"__new__\" " . selected($user_id, '__new__', false) . ">" . __('[New User]', 'owbn-chronicle-manager') . "</option>\n";
    ...
    // Actual Email
    echo "<div class=\"owbn-user-info-field\">\n";
    echo "<label>" . __('Actual Email', 'owbn-chronicle-manager') . "<br>\n";
...
function owbn_render_ast_group($key, $value, $meta) {
    ...
    foreach ($value as $index => $entry) {
        ...
        echo "<option value=\"\">" . __('— Select —', 'owbn-chronicle-manager') . "</option>\n";
        echo "<option value=\"__new__\" " . selected($entry['user'] ?? '', '__new__', false) . ">" . __('[New User]', 'owbn-chronicle-manager') . "</option>\n";
        foreach ($users as $user) {
        ...
        echo "<div class=\"owbn-user-info-field\">\n";
        echo "<label>" . __('Actual Email', 'owbn-chronicle-manager') . "<br>\n";
        echo "<input type=\"email\" name=\"ast_list[{$index}][actual_email]\" value=\"" . esc_attr($entry['actual_email'] ?? '') . "\" class=\"regular-text\">\n";
        echo "</label>\n</div>\n";
        echo "<div class=\"owbn-user-info-field\">\n";
        echo "<label>" . __('Display Email', 'owbn-chronicle-manager') . "<br>\n";
        echo "<input type=\"email\" name=\"ast_list[{$index}][display_email]\" value=\"" . esc_attr($entry['display_email'] ?? '') . "\" class=\"regular-text\">\n";
        echo "</label>\n</div>\n";
        echo "</div>\n";
