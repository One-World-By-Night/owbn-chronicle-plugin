...
function owbn_save_coordinator_meta($post_id, $post)
{
    ...
    if (!isset($_POST['owbn_coordinator_nonce']) || 
        !wp_verify_nonce($_POST['owbn_coordinator_nonce'], 'owbn_coordinator_meta_nonce')) {
        return;
    }
    ...
    if (isset($_POST['coordinator_slug'])) {
        ...
        if (empty($existing_slug)) {
            update_post_meta($post_id, 'coordinator_slug', sanitize_text_field($_POST['coordinator_slug']));
        }
    }
    ...
    if (isset($_POST['coord_info']) && is_array($_POST['coord_info'])) {
        $cleaned = owbn_sanitize_user_info($_POST['coord_info']);
    ...
    if (isset($_POST['subcoord_list']) && is_array($_POST['subcoord_list'])) {
        ...
        $cleaned = owbn_sanitize_ast_group($_POST['subcoord_list'], $meta_fields);
